...
## Post-Completion Change: Recategorization Algorithm Rework

Tracked separately (base project + all prior post-completion changes above unchanged/COMPLETE). Source: user report "the recategorisaion scan is a total failure. we need to totally rework on the algorithm." Ingestion Worker Service only (retroactive re-scan `recategorize_unsure_from_precedent`) — investigation confirmed ingestion-time `categorize()` unaffected, no other unit touched.

- [x] Requirements Analysis — Complete (2026-08-28; `recategorization-algorithm-rework-requirements.md`; 7 FRs, 3 NFRs; 2 rounds of questions. Round 1 [6 questions]: both false-positive and false-negative failure modes reported [Q1=A+C]; scope confirmed retroactive-re-scan-only after investigation [Q2=C, resolved: `categorize()` shows zero `categorization_disagreements` since 2026-08-15, no comparable failure pattern]; broaden from single-precedent pairwise comparison to a fuller precedent pool [Q3=A]; add an independent LLM verification step [Q4=A]; root-cause investigated in parallel with proceeding to rework regardless [Q5=C, resolved: structural boilerplate-similarity issue in PayNow transfer descriptions, not primarily the loosened `.env` threshold]; precision over auto-apply coverage [Q6=A]. Round 2 [3 questions, triggered by user's Request Changes feedback during review about embedding construction]: shared `build_embedding_text` function reworked globally across all 3 consumers [Q1=A], in-flow/out-flow added as an embedded signal alongside the existing price bucket rather than a hard pre-filter [Q2=A], reference codes left unstripped in the raw text, relying on the new LLM verification gate instead [Q3=B] — surfaced a new risk (NFR-RAR-3, stale stored vectors post-format-change, no existing backfill mechanism) for Functional Design to resolve. Live evidence gathered from `recategorization_jobs`/`recategorization_proposals`/`categorization_disagreements` tables before drafting questions and again before finalizing requirements — 100% proposal rejection rate 2026-08-23 through 08-27, root cause traced to boilerplate-heavy PayNow descriptions defeating single-precedent similarity matching.
- [x] User Stories — Skipped (2026-08-28; `recategorization-algorithm-rework-user-stories-assessment.md` [Execute=No]; pure backend accuracy rework, zero UI/workflow change, single stakeholder, ambiguity already resolved via 2 rounds of Requirements Analysis Q&A -- matches precedent set by `recategorization-scope-narrowing-requirements.md` and Matching Precision Refinement, both of which also skipped User Stories for identical reasoning)
- [ ] Workflow Planning — Complete, pending approval (2026-08-28, revised; `recategorization-algorithm-rework-execution-plan.md`; scope narrowed after user Request Changes -- LLM verification gate [FR-RAR-2] deferred to a documented future enhancement, not built now; rework is now pure embedding/matching-accuracy [broader precedent pool FR-RAR-1, embedding-text rework w/ direction signal + ID-stripping FR-RAR-6/7, threshold reconciliation FR-RAR-5]; Application Design SKIP [no new component/service], Units Generation SKIP [single existing unit -- Ingestion Worker Service]; Functional Design EXECUTE [precedent-pool retrieval strategy, ID-stripping heuristic, threshold calibration, NFR-RAR-3 backfill decision], NFR Requirements/NFR Design/Infrastructure Design SKIP, Code Generation + Build and Test ALWAYS; Risk: Low-Medium [revised down from Medium -- LLM-dependency risk factor removed; remaining risk is shared-function blast radius across 3 consumers + live production data])

- [x] Workflow Planning — Complete & Approved (2026-08-28)

## INCEPTION PHASE (this change): COMPLETE

### 🟢 CONSTRUCTION PHASE (this change, per-unit loop)
- [x] Ingestion Worker Service — Functional Design: Complete & Approved (2026-08-28, revised; `business-rules.md` WR-35..39 added [broadened precedent pool, direction signal, delimiter-based ID stripping, threshold reconciliation, full re-embedding backfill] + a Deferred entry for the LLM gate; `business-logic-model.md` Categorization Engine + Embedding Manager sections updated; `domain-entities.md` unchanged. WR-39 revised per user clarification: full backfill DOES happen [following WR-32's precedent], with an explicit hard constraint that assigned categories must not change as a side effect -- verified this is already true by construction, documented as a requirement anyway.)
  - [x] Ingestion Worker Service — Code Generation: Complete & Approved (2026-08-28; `recategorization-algorithm-rework-code-generation-plan.md`, all 8 steps; `embedding/text.py`/`categorization/service.py`/`orchestrator/pipeline.py`/`recurring_payments/service.py`/`embedding/service.py`/`.env` modified; new migration `0016`; 4 test files updated/extended. NOT verified by actually running the suite -- this machine's ingestion-worker/.venv has a broken Python symlink; verified via py_compile + repo-wide call-site grep instead. Fixing the venv and real test execution deferred to Build and Test.)

**UNIT: INGESTION WORKER SERVICE — COMPLETE (for this feature)**
**ALL UNITS COMPLETE (for this feature — only Ingestion Worker Service was affected) — proceeding to Build and Test**

## CONSTRUCTION PHASE (Recategorization Algorithm Rework): Code Generation complete

- [x] Build and Test — Complete (2026-08-29; `recategorization-algorithm-rework-build-and-test-summary.md`; fixed broken ingestion-worker/.venv on this machine; 291/291 unit tests passing, ruff clean, `docker compose build` clean; live-verified against real oMLX/Qdrant infra via rolled-back placeholder transaction -- exact reported false-positive shape confirmed fixed [zero match, previously always PENDING], genuine near-duplicate confirmed still auto-applies [98.77, no overcorrection]; redeployed for real, migration 0016 applied [alembic_version=0016], WR-39 backfill confirmed in progress [6096+14 rows reset to pending, 250 already re-completed within 15s, zero errors], categories confirmed untouched by construction; all 5 live containers healthy)

**Recategorization Algorithm Rework: COMPLETE** — build and test approved 2026-08-29; ingestion-worker live and healthy on `fix/review-page-flow-direction`'s working tree, not yet committed to git. Embedding backfill (WR-39) continuing in the background via the existing poll-cycle mechanism.
