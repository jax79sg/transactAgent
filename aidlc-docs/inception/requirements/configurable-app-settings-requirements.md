...
## Post-Approval Change (2026-08-16, discovered from user feedback after Build and Test): Three Real Defects Fixed

Live Build and Test verification was incomplete: it confirmed the mechanism worked for a setting with no pre-existing `.env` customization, but the user's real deployment had several settings already customized via `.env` directly (the pre-existing mechanism), and the user caught what that exposed. Three distinct, genuine defects, not scope changes:

1. **Stale displayed value, not the deployment's real one**: `GET /settings/openrouter_model` reported `"openrouter/free"` (the catalog's hardcoded Python default) when the real deployment's `.env` had it set to `"gemma-4-26b-a4b-it-4bit"`. Root cause: the original `_effective_value_str` fell back to a static catalog default for anything not yet overridden through this feature's own UI — exactly the "Known Limitation" the original Build and Test summary had documented but not treated as blocking. The user's report showed this wasn't an acceptable edge case to leave documented-but-unfixed. **Fix**: `api-service`'s `Settings` class gained a display-only mirror of every Ingestion-Worker-owned field, fed the identical `docker-compose.yml` environment entries Ingestion Worker itself receives, so both services' `Settings` objects agree by construction (`business-rules.md` AR-34).
2. **No per-setting description or grouping**: the Settings page showed only a bare name and a raw value — no explanation of what a setting does, despite `.env.example` already having rich, well-organized explanatory comments and section headers for nearly every field. **Fix**: every `SettingSpec` gained a `category` (matching `.env.example`'s own section organization: Matching & Categorization / Embedding & Semantic Matching / Recurring Payments / Backup / Ingestion / API & Access / Ask AI) and a `description` sourced directly from that same `.env.example`/`config.py` text — the Settings page now groups by category and shows the real description per row, not a generic label.
3. **A genuinely missing setting**: `ai_assistant_max_transactions` was named in the original Requirements "Expose" table (Current Behavior/Resolved Decision 1's row list) but was never actually added to the implementation's allow-list at all — found while fixing the two issues above, not related to either. True count is **41**, not 40 (see `business-rules.md` AR-28's correction note).

No FR/NFR wording changed as a result — `FR-CAS-3` ("current effective value") already required accuracy; the fix corrects an implementation gap against that existing requirement, not a new one.
